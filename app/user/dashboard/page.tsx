...
export default function DashboardComponent() {
  ...
  return ( <>  <div className=" md:hidden shrink-0 border-y py-4 border-gray-300">
    <h1 className="text-[16px] font-medium text-[#1C1C1D] px-6 mb-1.5 tracking-tight">
    Home
    </h1>
    ...
    <div className="w-full font-dm-sans px-6 lg:px-8 py-8">
      ...
      <div className="inline-flex items-center gap-3 border border-[#EAECF0] rounded-full p-1 pr-4 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.02)] w-max">
        ...
        <span className=" text-[12px] md:text-[14px] text-[#525866] font-medium">
          Introducing the Submit Right
        </span>
      </div>

      <div className="mt-6 mb-8">
        <p className="text-[#A0AAB5] text-[12px] md:text-[14px] font-medium mb-1">
          My Workspace
        </p>
        <h1 className="text-[14px] md:text-[32px] font-medium text-[#171717] tracking-tight">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 mb-12">
        {dashboardData.services.map((service) => (
          <div key={service.id} className="flex flex-col items-center gap-4">
            ...
            <p className="text-center text-[12px] md:text-[14px] font-semibold text-[#171717] whitespace-pre-line leading-tight">
              {service.title}
            </p>
          </div>
        ))}
      ...
      <div className="mb-16">
        <h2 className="text-[18px] md:text-[24px] font-semibold text-[#171717] mb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6">
          {dashboardData.packages.map((pkg) => (
            ...
            <div
            key={pkg.id}
            className="bg-[#F5F7FA] rounded-[24px] p-6 sm:p-8 flex flex-col justify-end min-h-[280px]"
          >
            <div>
              ...
              {/* Removed 'mx-2' so bullets align perfectly with the title on mobile */}
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6">
                {/* Reduced mobile horizontal gap (gap-x-2) so the two columns aren't too far apart */}
                <div className="grid grid-cols-2 gap-x-2 gap-y-3 sm:gap-x-8 sm:gap-y-2 w-full sm:w-auto">
                  {pkg.features.map((feature, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 text-[#525866] text-[13px] sm:text-[14px]"
                    >
                      <div className="w-[4px] h-[4px] bg-[#A0AAB5] rounded-full shrink-0"></div>
                      <span className="truncate sm:whitespace-normal">{feature}</span>
                    </div>
                  ))}
                </div>
                <button className="hidden sm:block bg-[#00A0E3] hover:bg-[#008bc5] text-white px-6 py-2.5 rounded-[8px] text-[14px] font-bold transition-colors shrink-0">
                  {pkg.buttonText}
                </button>
              </div>
            </div>
      ...
      <div className="mb-16 relative w-full overflow-hidden">
        <h2 className=" text-[18px] md:text-[20px] font-semibold text-[#171717] mb-8">
      ...
      <div className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[12px] md:gap-6">
            ...
            >
              ...
              <div className="flex items-end justify-between mb-6">
                <div className="flex flex-col">
                  <span className="text-[14px] text-[#A0AAB5] mb-1">Starts from</span>
                  <div>
                    <span className="text-[16px] md:text-[22px] font-semibold text-[#171717]">{card.price}</span>
                    <span className= "text-[12px] md:text-[13px] text-[#A0AAB5] ml-1">{card.unit}</span>
                  </div>
                </div>
                <button className="bg-[#00A0E3] hover:bg-[#008bc5] text-white px-4 md:px-5 py-2.5 rounded-[8px] text-[14px] font-medium transition-colors">
                  {card.buttonText}
                </button>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
          {additionalData.exploreCards.map((card) => (
            <div key={card.id} className="border border-[#EAECF0] rounded-[12px] overflow-hidden bg-white flex flex-col shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
              <div className={` h-[180px] md:h-[160px] w-full ${card.bgColor}`}></div>
              <div className="p-5 flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-[16px] font-semibold text-[#171717]">{card.title}</h3>
                ...
                <button className="w-full py-2 border border-[#00A0E3] text-[#00A0E3] rounded-[8px] text-[14px] font-bold hover:bg-[#F0F8FC] transition-colors">
                  Explore
                </button>
              </div>
            </div>
      ...
      {/* --- YOUR ORDERS SECTION --- */}
      <div className="mb-12">
        <h2 className=" text-[18px] text-[20px] font-semibold text-[#171717] mb-6">
          Your Orders
        </h2>
        <div className="bg-[#F0F7FB] border border-[#E4EEF5] rounded-[16px] p-4 md:p-8">
      ...
      <div className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border border-[#EAECF0] rounded-[16px] bg-white lg:divide-x divide-y lg:divide-y-0 divide-[#EAECF0] overflow-hidden">
          {additionalData.reasons.map((reason) => (
            <div key={reason.id} className="p-4 md:p-8 flex flex-row md:flex-col items-center md:items-start">
            <div className="w-[72px] h-[72px] rounded-full bg-[#F5F7FA] flex-shrink-0 mr-4 mb-0 md:mr-0 md:mb-6"></div>
            <div>
              <h3 className="text-[14px] md:text-[16px] font-semibold text-[#171717] mb-1 md:mb-2">
                {reason.title}
              </h3>

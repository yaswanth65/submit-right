...
export default function ServiceDetailsPage() {
  ...
  const sections = ["Hero Section", "Introduction", "Why Choose", "Contact", "FAQs", "CTA Banner"];
  ...
  const loadItem = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : "Unable to load service details");
    } finally {
      setIsLoading(false);
    }
  }, [itemId]);
  ...
  const titleLabel = item?.kind === "domain" ? "Domain Name:" : "Service Name:";
  const nameValue = item?.title ?? "-";
  const domainType = item?.domainType ?? item?.category ?? "-";
  const priceLabel = item?.kind === "service" ? "Word Count Rate:" : "Base Price:";
  const priceValue =
    item?.kind === "service"
      ? item?.ratePerWord != null
        ? `${formatCurrency(item.ratePerWord)} / word`
        : "-"
      : formatCurrency(item?.basePrice ?? null);
    ...
    <div className="w-full font-dm-sans">
      <div className="mb-8 flex items-center gap-3">
        <Link
          ...
          className="flex items-center justify-center rounded-md p-1.5 text-[#1C1C1D] transition-colors hover:bg-gray-100"
        >
      ...
      <div className="mb-6 rounded-[12px] border border-[#EAECF0] bg-white p-6 shadow-sm">
        <div className="mb-6 flex items-center justify-between">
          <div className="font-inter text-[18px] font-bold text-[#1C1C1D]">Service Details</div>
          <button
        ...
        {error ? (
          ...
          <div className="flex flex-col gap-8 md:flex-row">
            ...
            <div className="grid flex-1 grid-cols-1 items-start gap-y-6 md:grid-cols-2">
              <div>
                <p className="mb-1 text-[13px] text-[#78788D]">{titleLabel}</p>
              ...
              <div>
                <p className="mb-1 text-[13px] text-[#78788D]">{priceLabel}</p>
                <p className="text-[15px] font-medium text-[#1C1C1D]">{priceValue}</p>
              </div>

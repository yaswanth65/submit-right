// app/user/components/MobileSidebar.tsx
"use client";
...
export function MobileSidebar({ isOpen, setIsOpen }: { isOpen: boolean; setIsOpen: (val: boolean) => void }) {
  ...
  return (
    <>
      ...
      <aside 
        className={`fixed inset-y-0 left-0 z-[70] w-[260px] bg-[#FAFAFB] flex flex-col md:hidden transform transition-transform duration-300 ease-in-out border-r border-[#E5E7EB] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ${
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <div key={item.name}>
                      ...
                      {item.hasSub && isOpenMenu && item.subItems && (
